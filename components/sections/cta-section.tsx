'use client'

import Link from 'next/link'
import { useEffect, useRef } from 'react'

const WA_ICON = (
  <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 flex-shrink-0">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413Z" />
  </svg>
)

export function CtaSection() {
  const sectionRef = useRef<HTMLElement>(null)

  useEffect(() => {
    const el = sectionRef.current
    if (!el) return
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          el.classList.add('cta-visible')
          observer.unobserve(el)
        }
      },
      { threshold: 0.1 }
    )
    observer.observe(el)
    return () => observer.disconnect()
  }, [])

  return (
    <section ref={sectionRef} className="cta-reveal py-16 bg-white">
      <div className="mx-auto max-w-6xl px-4">
        <div className="overflow-hidden rounded-3xl flex flex-col md:flex-row">

          {/* Left 40%: photo with red overlay */}
          <div className="relative h-48 md:h-auto md:w-[40%] flex-shrink-0">
            <img
              src="https://lovidostavka.kz/media/hero/hero.webp"
              alt="Курьер на мотоцикле"
              className="absolute inset-0 h-full w-full object-cover"
            />
            <div className="absolute inset-0 bg-primary/60" />
          </div>

          {/* Right 60%: dark bg with content */}
          <div className="flex flex-col justify-center bg-[#1A1A1A] px-8 py-10 md:px-12 md:py-14 md:w-[60%]">

            {/* Badge */}
            <div className="mb-6 inline-flex w-fit items-center gap-2 rounded-full bg-primary px-4 py-1.5 text-xs font-bold text-white">
              🔴 Подключение за 5 минут
            </div>

            <h2 className="mb-4 text-3xl font-black leading-tight text-white lg:text-4xl">
              Оставьте заявку на подключение к KanEXPRESS!
            </h2>

            <p className="mb-8 text-white/50 leading-relaxed">
              Начните экономить на доставке уже сегодня.<br />
              Первые 50 заказов — бесплатно.
            </p>

            <div className="flex flex-col gap-3 sm:flex-row">
              <a
                href="https://wa.me/77767286272"
                className="wa-pulse inline-flex items-center justify-center gap-2.5 rounded-xl bg-[#25D366] px-6 py-3.5 text-sm font-bold text-white transition-transform hover:scale-105 active:scale-95 w-full sm:w-auto"
              >
                {WA_ICON}
                Написать в WhatsApp
              </a>
              <Link
                href="/register"
                className="inline-flex items-center justify-center rounded-xl border border-white/20 px-6 py-3.5 text-sm font-medium text-white/70 transition-all hover:scale-105 hover:border-white/40 hover:text-white w-full sm:w-auto"
              >
                Зарегистрироваться
              </Link>
            </div>
          </div>

        </div>
      </div>
    </section>
  )
}
